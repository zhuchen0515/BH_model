...
"""
Calculate the cooling rate with different methods
...
## calculate the cooling rate with fornula from Sandy's script P.7
## This formula is equal to ClassCool function when omega_l = 0
def v_lmcool_p7(lm_hz,z):
     lmcool = 10+np.log10(1.9)+1.5*np.log10(fhot)+(5.0*(lm_hz+lsun)/12.0)+\
              (11.0*np.log10(1+z)/8.0)
     lmcool = lmcool-lsun+lyrtosec
     return lmcool

## Initialize the halo properties
class ClassHaloP(InitCosmos):
     def __init__(self,omega_m=0.3,omega_l=0.7,omega_b=0.048,h0=0.7):
          InitCosmos.__init__(self,omega_m=omega_m,omega_l=omega_l,omega_b=omega_b,h0=h0)
